...
export const TaskBody = (props: Props) => {
  ...
  const handleChangeDeadlineDate = (event: React.ChangeEvent<HTMLInputElement>) => {
  ...
  return (
    <form className="modal_body">
      ...
      <div>
        ...
        <input className="input_date" type="date" value={deadlineDate} onChange={handleChangeDeadlineDate} />
      </div>
      <input className="input_submit" type="button" value="送信" onClick={onClickSubmit} />
      { props.task && (
        <button className="button delete_button" type="button" onClick={handleOnDelete}>このタスクを削除する</button>
      ) }
    </form>
  );
};